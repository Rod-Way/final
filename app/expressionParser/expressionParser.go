...
func CalcExpression(node *Node, counter *int, waitg *sync.WaitGroup, log *slog.Logger, client *http.Client, agentChan chan api.Agent) {
	defer waitg.Done()
	if node == nil {
		return
	}
...
func PostTask(log *slog.Logger, exprassion string, client *http.Client, agentChan chan api.Agent) (float64, error) {
...
func ValidatedPostOrder(ctx context.Context, log *slog.Logger, expression string, client *http.Client, agentChan chan api.Agent) (float64, error) {
